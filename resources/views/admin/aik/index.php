<style>
    @keyframes aikListShine { 0%{transform:translateX(-100%) skewX(-20deg)} 100%{transform:translateX(300%) skewX(-20deg)} }
    ...
    .btn-add-3d::after {
        ...
        animation: aikListShine 3s ease-in-out infinite;
    }
...
<!-- ================= HEADER 3D ================= -->
<div class="aik-list-head">
    <div style="display: flex; align-items: center; gap: 16px; position: relative; z-index: 1;">
        <div class="aik-list-ico">🕌</div>
        <div>
            <h2 class="aik-list-title">Kegiatan AIK / Catur Dharma</h2>
            <span class="aik-list-count">📊 Total <?= (int) $total ?> data</span>
        </div>
    </div>
    <a class="btn-add-3d" href="<?= e(url('admin/index.php?page=aik-tambah')) ?>">➕ Tambah Kegiatan</a>
</div>
...
<form method="get" action="<?= e(url('admin/index.php')) ?>" class="filter-3d">
    <input type="hidden" name="page" value="aik">
    <input type="text" name="q" value="<?= e($filters['q']) ?>" placeholder="🔍 Cari judul/lokasi...">
...
<div style="overflow-x: auto;">
    <table class="table-3d">
        ...
        <tbody>
            <?php if ($items === []): ?>
                <tr><td colspan="6"><div class="empty-3d"><span>🕌</span>Belum ada data kegiatan AIK.</div></td></tr>
            <?php endif; ?>

            <?php foreach ($items as $item): ?>
                <tr>
                    <td class="td-title-3d"><?= e($item['title']) ?></td>
                    <td><span class="cat-badge-3d">🕌 <?= e(AikActivity::CATEGORIES[$item['category']] ?? $item['category']) ?></span></td>
                    <td>📅 <?= e(date('d M Y', strtotime($item['activity_date']))) ?></td>
                    ...
                    <td>
                        <div style="display: flex; gap: 6px;">
                            <a class="btn-edit-3d" href="<?= e(url('admin/index.php?page=aik-edit&id=' . (int) $item['id'])) ?>">✏️ Edit</a>
                            <form method="post" action="<?= e(url('admin/index.php?page=aik-hapus')) ?>" class="js-confirm" data-message="Yakin hapus data ini?">
...
<?php if ($totalPages > 1): ?>
    <div class="pag-3d">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="<?= e(url('admin/index.php?page=aik&hal=' . $i . ($filters['q'] !== '' ? '&q=' . urlencode($filters['q']) : '') . ($filters['category'] !== '' ? '&category=' . urlencode($filters['category']) : '') . ($filters['status'] !== '' ? '&status=' . urlencode($filters['status']) : ''))) ?>" class="<?= $i === $page ? 'active' : '' ?>"><?= $i ?></a>
        <?php endfor; ?>
    </div>
<?php endif; ?>