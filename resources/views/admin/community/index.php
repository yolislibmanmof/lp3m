<style>
    @keyframes csListShine { 0%{transform:translateX(-100%) skewX(-20deg)} 100%{transform:translateX(300%) skewX(-20deg)} }
    ...
    .cs-list-ico {
        ...
        box-shadow: inset 0 2px 3px rgba(255,255,255,0.6), inset 0 -3px 4px rgba(0,0,0,0.25), 0 6px 14px rgba(5,150,105,0.4);
    }
    .cs-list-ico::before {
    ...
    .btn-add-3d::after {
        ...
        animation: csListShine 3s ease-in-out infinite;
    }
    ...
    .table-3d tbody tr:hover td { background: rgba(5,150,105,0.04); }
    .td-title-3d { font-weight: 800; color: var(--ink); }
    .type-badge-3d {
        display: inline-flex; padding: 4px 11px; border-radius: 999px;
        font-size: 11px; font-weight: 800;
        background: linear-gradient(145deg, rgba(52,211,153,0.15), rgba(5,150,105,0.1));
        border: 1px solid rgba(5,150,105,0.3); color: #047857;
    }
    .aik-badge-3d {
        ...
        color: #fff;
        box-shadow: inset 0 1px 2px rgba(255,255,255,0.4), inset 0 -1px 2px rgba(0,0,0,0.2), 0 2px 6px rgba(124,58,237,0.35);
    }
    .status-jewel-3d {
        display: inline-flex; align-items: center; gap: 6px;
    ...
    .pag-3d a:hover { transform: translateY(-2px); border-color: rgba(5,150,105,0.3); }
    .pag-3d a.active {
...
<div class="cs-list-head">
    <div style="display: flex; align-items: center; gap: 16px; position: relative; z-index: 1;">
        <div class="cs-list-ico">🤝</div>
        <div>
            <h2 class="cs-list-title">Pengabdian & KKN</h2>
            <span class="cs-list-count">📊 Total <?= (int) $total ?> data</span>
        </div>
    </div>
    <a class="btn-add-3d" href="<?= e(url('admin/index.php?page=pengabdian-tambah')) ?>">➕ Tambah Kegiatan</a>
</div>
...
<form method="get" action="<?= e(url('admin/index.php')) ?>" class="filter-3d">
    <input type="hidden" name="page" value="pengabdian">
    <input type="text" name="q" value="<?= e($filters['q']) ?>" placeholder="🔍 Cari judul/ketua/lokasi...">
...
<div style="overflow-x: auto;">
    <table class="table-3d">
        ...
        <tbody>
            <?php if ($items === []): ?>
                <tr><td colspan="7"><div class="empty-3d"><span>🤝</span>Belum ada data pengabdian.</div></td></tr>
            <?php endif; ?>
                ...
                <tr>
                    <td class="td-title-3d">
                        ...
                        <?php if (!empty($item['aik_integration'])): ?>
                            <span class="aik-badge-3d">🕌 AIK</span>
                        <?php endif; ?>
                    </td>
                    <td><span class="type-badge-3d">🤝 <?= e(CommunityService::TYPES[$item['type']] ?? $item['type']) ?></span></td>
                    <td>👤 <?= e($item['leader']) ?></td>
                    <td>📍 <?= e($item['location']) ?></td>
                    <td><strong style="color: var(--gold-strong); font-family: var(--font-display);"><?= (int) $item['year'] ?></strong></td>
                    <td>
                    ...
                    <td>
                        <div style="display: flex; gap: 6px;">
                            <a class="btn-edit-3d" href="<?= e(url('admin/index.php?page=pengabdian-edit&id=' . (int) $item['id'])) ?>">✏️ Edit</a>
                            <form method="post" action="<?= e(url('admin/index.php?page=pengabdian-hapus')) ?>" class="js-confirm" data-message="Yakin hapus data ini?">
...
<?php if ($totalPages > 1): ?>
    <div class="pag-3d">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="<?= e(url('admin/index.php?page=pengabdian&hal=' . $i . ($filters['q'] !== '' ? '&q=' . urlencode($filters['q']) : '') . ($filters['type'] !== '' ? '&type=' . urlencode($filters['type']) : '') . ($filters['status'] !== '' ? '&status=' . urlencode($filters['status']) : ''))) ?>" class="<?= $i === $page ? 'active' : '' ?>"><?= $i ?></a>
        <?php endfor; ?>
    </div>
<?php endif; ?>