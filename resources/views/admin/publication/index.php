<style>
    @keyframes pubListShine { 0%{transform:translateX(-100%) skewX(-20deg)} 100%{transform:translateX(300%) skewX(-20deg)} }
    ...
    .pub-list-ico {
        ...
        box-shadow: inset 0 2px 3px rgba(255,255,255,0.6), inset 0 -3px 4px rgba(0,0,0,0.25), 0 6px 14px rgba(124,58,237,0.4);
    }
    .pub-list-ico::before {
    ...
    .btn-add-3d::after {
        ...
        animation: pubListShine 3s ease-in-out infinite;
    }
    ...
    .table-3d tbody tr:hover td { background: rgba(124,58,237,0.04); }
    .td-title-3d { font-weight: 800; color: var(--ink); }

    .type-badge-3d {
        display: inline-flex; padding: 4px 11px; border-radius: 999px;
        font-size: 11px; font-weight: 800;
        background: linear-gradient(145deg, rgba(167,139,250,0.15), rgba(124,58,237,0.1));
        border: 1px solid rgba(124,58,237,0.3); color: #6d28d9;
    }

    /* Level badge cerdas berdasarkan reputasi */
    .level-badge-3d {
    ...
    .lv-intl { background: linear-gradient(145deg, #fde68a, #d9a441); color: #03251f; box-shadow: inset 0 1px 2px rgba(255,255,255,0.7), 0 2px 6px rgba(217,164,65,0.3); }
    .lv-sinta12 { background: linear-gradient(145deg, #6ee7b7, #10b981); color: #064e3b; box-shadow: inset 0 1px 2px rgba(255,255,255,0.6); }
    .lv-sinta { background: linear-gradient(145deg, #93c5fd, #3b82f6); color: #fff; box-shadow: inset 0 1px 2px rgba(255,255,255,0.5); }
    .lv-nas { background: linear-gradient(145deg, #5eead4, #14b8a6); color: #134e4a; box-shadow: inset 0 1px 2px rgba(255,255,255,0.6); }
    .lv-default { background: linear-gradient(145deg, #e5e7eb, #9ca3af); color: #374151; box-shadow: inset 0 1px 2px rgba(255,255,255,0.6); }
    ...
    .pag-3d a:hover { transform: translateY(-2px); border-color: rgba(124,58,237,0.3); }
    .pag-3d a.active {
...
<?php
// Deteksi level publikasi untuk badge cerdas
function pubLevelClass(string $level): array {
    $l = strtolower($level);
    if (strpos($l, 'scopus') !== false || strpos($l, 'web of science') !== false
        || strpos($l, 'wos') !== false || strpos($l, 'q1') !== false || strpos($l, 'q2') !== false
        || strpos($l, 'internasional') !== false) {
        return ['lv-intl', '⭐'];
    }
    if (strpos($l, 'sinta 1') !== false || strpos($l, 'sinta 2') !== false) {
        return ['lv-sinta12', '🏆'];
    }
    if (strpos($l, 'sinta') !== false) {
        return ['lv-sinta', '📘'];
    }
    if (strpos($l, 'nasional') !== false) {
        return ['lv-nas', '🇮🇩'];
    }
    return ['lv-default', '📄'];
}
...
<div class="pub-list-head">
    <div style="display: flex; align-items: center; gap: 16px; position: relative; z-index: 1;">
        <div class="pub-list-ico">📚</div>
        <div>
            <h2 class="pub-list-title">Publikasi Ilmiah</h2>
            <span class="pub-list-count">📊 Total <?= (int) $total ?> data</span>
        </div>
    </div>
    <a class="btn-add-3d" href="<?= e(url('admin/index.php?page=publikasi-tambah')) ?>">➕ Tambah Publikasi</a>
</div>
...
<form method="get" action="<?= e(url('admin/index.php')) ?>" class="filter-3d">
    ...
    <input type="text" name="q" value="<?= e($filters['q']) ?>" placeholder="🔍 Cari judul/penulis...">
...
<div style="overflow-x: auto;">
    <table class="table-3d">
        ...
        <tbody>
            <?php if ($items === []): ?>
                <tr><td colspan="6"><div class="empty-3d"><span>📚</span>Belum ada data publikasi.</div></td></tr>
            <?php endif; ?>

            <?php foreach ($items as $item):
                $lv = pubLevelClass($item['level'] ?? '');
            ?>
                <tr>
                    <td class="td-title-3d"><?= e($item['title']) ?></td>
                    <td><span class="type-badge-3d">📖 <?= e(Publication::TYPES[$item['type']] ?? $item['type']) ?></span></td>
                    <td>
                        <span class="level-badge-3d <?= $lv[0] ?>">
                            <?= $lv[1] ?> <?= e($item['level']) ?>
                        </span>
                    </td>
                    ...
                    <td>
                        <div style="display: flex; gap: 6px;">
                            <a class="btn-edit-3d" href="<?= e(url('admin/index.php?page=publikasi-edit&id=' . (int) $item['id'])) ?>">✏️ Edit</a>
                            <form method="post" action="<?= e(url('admin/index.php?page=publikasi-hapus')) ?>" class="js-confirm" data-message="Yakin hapus data ini?">
...
<?php if ($totalPages > 1): ?>
    <div class="pag-3d">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="<?= e(url('admin/index.php?page=publikasi&hal=' . $i . ($filters['q'] !== '' ? '&q=' . urlencode($filters['q']) : '') . ($filters['type'] !== '' ? '&type=' . urlencode($filters['type']) : '') . ($filters['status'] !== '' ? '&status=' . urlencode($filters['status']) : ''))) ?>" class="<?= $i === $page ? 'active' : '' ?>"><?= $i ?></a>
        <?php endfor; ?>
    </div>
<?php endif; ?>