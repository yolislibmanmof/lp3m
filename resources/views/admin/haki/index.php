<style>
    @keyframes hakiListShine { 0%{transform:translateX(-100%) skewX(-20deg)} 100%{transform:translateX(300%) skewX(-20deg)} }
    ...
    .haki-list-ico {
        ...
        box-shadow: inset 0 2px 3px rgba(255,255,255,0.6), inset 0 -3px 4px rgba(0,0,0,0.25), 0 6px 14px rgba(59,130,246,0.4);
    }
    .haki-list-ico::before {
    ...
    .btn-add-3d::after {
        ...
        animation: hakiListShine 3s ease-in-out infinite;
    }
    ...
    .table-3d tbody tr:hover td { background: rgba(59,130,246,0.04); }
    .td-title-3d { font-weight: 800; color: var(--ink); }

    .type-badge-3d {
        display: inline-flex; padding: 4px 11px; border-radius: 999px;
        font-size: 11px; font-weight: 800;
        background: linear-gradient(145deg, rgba(96,165,250,0.15), rgba(59,130,246,0.1));
        border: 1px solid rgba(59,130,246,0.3); color: #1e40af;
    }
    .regnum-3d {
        font-family: 'Courier New', monospace; font-size: 12.5px; font-weight: 700;
        color: var(--primary-dark); padding: 4px 10px; border-radius: 8px;
        background: rgba(5,150,105,0.08); border: 1px solid rgba(5,150,105,0.2);
    }
    .regnum-empty { color: var(--muted); font-style: italic; }

    /* 5 Status Jewels untuk HAKI pipeline */
    .status-jewel-3d {
        display: inline-flex; align-items: center; gap: 5px;
        padding: 4px 12px; border-radius: 999px;
        font-size: 10.5px; font-weight: 800; position: relative; overflow: hidden;
        white-space: nowrap;
    }
    ...
    .sj-review { background: linear-gradient(145deg, #fde68a, #d9a441); color: #03251f; box-shadow: inset 0 1px 2px rgba(255,255,255,0.7), inset 0 -1px 2px rgba(0,0,0,0.15); }
    .sj-djki { background: linear-gradient(145deg, #bfdbfe, #3b82f6); color: #fff; box-shadow: inset 0 1px 2px rgba(255,255,255,0.5), inset 0 -1px 2px rgba(0,0,0,0.15); }
    .sj-perbaikan { background: linear-gradient(145deg, #fed7aa, #f97316); color: #7c2d12; box-shadow: inset 0 1px 2px rgba(255,255,255,0.6), inset 0 -1px 2px rgba(0,0,0,0.15); }
    .sj-terbit { background: linear-gradient(145deg, #6ee7b7, #10b981); color: #064e3b; box-shadow: inset 0 1px 2px rgba(255,255,255,0.6), inset 0 -1px 2px rgba(0,0,0,0.15); }

    .year-3d { font-family: var(--font-display); font-weight: 900; font-size: 14px; color: var(--gold-strong); }
    ...
    .pag-3d a:hover { transform: translateY(-2px); border-color: rgba(59,130,246,0.3); }
    .pag-3d a.active {
...
<?php
// Mapping status ke class jewel
$statusJewelMap = [
    'draft_internal' => ['sj-draft', '📝'],
    'review_internal' => ['sj-review', '🔍'],
    'diajukan_djki' => ['sj-djki', '📤'],
    'perbaikan' => ['sj-perbaikan', '✏️'],
    'sertifikat_terbit' => ['sj-terbit', '🏅'],
];
?>

<!-- ================= HEADER 3D ================= -->
<div class="haki-list-head">
    <div style="display: flex; align-items: center; gap: 16px; position: relative; z-index: 1;">
        <div class="haki-list-ico">🛡️</div>
        <div>
            <h2 class="haki-list-title">HAKI / Kekayaan Intelektual</h2>
            <span class="haki-list-count">📊 Total <?= (int) $total ?> data</span>
        </div>
    </div>
    <a class="btn-add-3d" href="<?= e(url('admin/index.php?page=haki-tambah')) ?>">➕ Tambah HAKI</a>
</div>
...
<form method="get" action="<?= e(url('admin/index.php')) ?>" class="filter-3d">
    <input type="hidden" name="page" value="haki">
    <input type="text" name="q" value="<?= e($filters['q']) ?>" placeholder="🔍 Cari judul/inventor...">
...
<div style="overflow-x: auto;">
    <table class="table-3d">
        ...
        <tbody>
            <?php if ($items === []): ?>
                <tr><td colspan="6"><div class="empty-3d"><span>🛡️</span>Belum ada data HAKI.</div></td></tr>
            <?php endif; ?>

            <?php foreach ($items as $item):
                $sj = $statusJewelMap[$item['status']] ?? ['sj-draft', '📝'];
            ?>
                <tr>
                    <td class="td-title-3d"><?= e($item['title']) ?></td>
                    <td><span class="type-badge-3d">🛡️ <?= e(IntellectualProperty::TYPES[$item['type']] ?? $item['type']) ?></span></td>
                    <td>
                        <?php if (!empty($item['registration_number'])): ?>
                            <span class="regnum-3d">🔢 <?= e($item['registration_number']) ?></span>
                        <?php else: ?>
                            <span class="regnum-empty">— Belum ada —</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <span class="status-jewel-3d <?= $sj[0] ?>">
                            <?= $sj[1] ?> <?= e(IntellectualProperty::STATUSES[$item['status']] ?? $item['status']) ?>
                        </span>
                    </td>
                    <td><span class="year-3d"><?= (int) $item['year'] ?></span></td>
                    <td>
                        <div style="display: flex; gap: 6px;">
                            <a class="btn-edit-3d" href="<?= e(url('admin/index.php?page=haki-edit&id=' . (int) $item['id'])) ?>">✏️ Edit</a>
                            <form method="post" action="<?= e(url('admin/index.php?page=haki-hapus')) ?>" class="js-confirm" data-message="Yakin hapus data ini?">
...
<?php if ($totalPages > 1): ?>
    <div class="pag-3d">
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="<?= e(url('admin/index.php?page=haki&hal=' . $i . ($filters['q'] !== '' ? '&q=' . urlencode($filters['q']) : '') . ($filters['type'] !== '' ? '&type=' . urlencode($filters['type']) : '') . ($filters['status'] !== '' ? '&status=' . urlencode($filters['status']) : ''))) ?>" class="<?= $i === $page ? 'active' : '' ?>"><?= $i ?></a>
        <?php endfor; ?>
    </div>
<?php endif; ?>